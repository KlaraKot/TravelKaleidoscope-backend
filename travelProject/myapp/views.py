...
class RefreshToken(APIView):
    def post(self, request):
        ...
        token2 = jwt.encode(payload2, 'secret', algorithm='HS256')#.decode('utf-8')\

        response2 = Response()

        response2.set_cookie(key='jwt', value=token2, httponly=True)
...
class TenBestView(APIView):
    def get(self, request):
        ...
        try:
            ...
            raters1 = tenBest['raters']
            raters = []
            for r in raters1:
                raters.append(r)

            score1 = tenBest['score']
            score = []
            for s in score1:
                score.append(s)
